...
function CreateCertificate($name, $dnsName){
    ...
    $cert = New-SelfSignedCertificate -FriendlyName $name -CertStoreLocation cert:\LocalMachine\My -DnsName $dnsName 
...
function SetupHttps($siteName, $relativePath, $port){
...
Clear-Host
#run!
SetupHttps "Bjmp" "Hosts\Bjmp.Web" 316
